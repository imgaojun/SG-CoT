#!/usr/bin/env python3
"""Freeze the two E120 category slices missing from the E119 transfer matrix."""
...
TARGET_CATEGORIES = ("event_omission", "trigger_drift")
...
def main() -> int:
    ...
    for category in TARGET_CATEGORIES:
        if counts[category] != args.expected_per_category:
            raise ValueError(
                f"{category}: expected {args.expected_per_category}, found {counts[category]}"
            )
        selected = [
        ...
        dataset_name = f"e120a_{category}"
        ...
        artifact_hashes[category] = sha256(output_path)
        manifest.append(
            {
                "category": category,
    ...
    frozen = {
        "frozen": True,
        "protocol": "E120 remaining single-category transfer experts",
