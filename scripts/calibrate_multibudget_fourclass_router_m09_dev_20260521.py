...
LIGHT_DEV = REPO / "outputs/stage2_adaptive_runs_user_devpick_frontier/richere_split1_qwen3_1_7b_instruct_oracle_mixed_noise_top10_shuffle_adaptive_confrare10_heur10_type_plan_lite_full_forced_reason_dev_seen_max512/checkpoint-1290/predictions.jsonl"
...
OUT_JSON = REPO / "reports/artifacts/2026-05-21_stage2_multibudget_fourclass_router_m09_dev.json"
OUT_MD = REPO / "reports/2026-05-21_stage2_multibudget_fourclass_router_m09_dev.md"

ROUTES = ["reason_light", "reason_mid", "reason_full"]
WINDOW_ENDPOINTS = [i / 40 for i in range(0, 21)]
TARGET_MIN_RATE = 0.04
TARGET_MAX_RATE = 0.25
...
def candidate_rows(route, direct_rows, light_rows, mid_rows, full_rows):
...
def evaluate_policy(name, ckpt, ordered_items, selected_items, score_rows, direct_rows, light_rows, mid_rows, full_rows):
    selected = {key: route for _, key, route in selected_items}
    keys = [key for _, key, _ in ordered_items]
    route_counts = {"direct": 0, "reason_light": 0, "reason_mid": 0, "reason_full": 0}
    routed_metrics = []
    direct_metrics = []
    selected_deltas = []
    correct = label_tp = label_fp = label_fn = 0
    ...
    for rank, key in enumerate(keys, start=1):
        ...
        chosen_row = candidate_rows(exec_route, direct_rows, light_rows, mid_rows, full_rows)[key]
        best_non_direct_row = candidate_rows(
            best_route_for_score_row(score_row), direct_rows, light_rows, mid_rows, full_rows
        )[key]
        ...
        if exec_route != "direct":
            selected_deltas.append(metric_delta(direct_row, chosen_row))
    ...
    return {
        ...
        "reason_count": sum(route_counts[r] for r in ROUTES),
        "reason_rate": sum(route_counts[r] for r in ROUTES) / len(keys) if keys else 0.0,
...
def add_fold_floor(row, ordered_items, selected_items, score_rows, direct_rows, light_rows, mid_rows, full_rows):
    selected = {(key, route) for _, key, route in selected_items}
    folds = []
    for fold in range(5):
...
def sweep(direct_rows, light_rows, mid_rows, full_rows, common_keys):
    rows = []
    for score_path in sorted(SCORE_ROOT.glob("checkpoint-*/dev_seen_scores.jsonl"), key=ckpt_num):
        ...
        for lo in WINDOW_ENDPOINTS:
            for hi in WINDOW_ENDPOINTS:
                ...
                rows.append(add_fold_floor(row, ordered, selected, score_rows, direct_rows, light_rows, mid_rows, full_rows))
    return rows
...
def balanced_score(row):
    ...
    return (min(d["argument_f1"], d["event_f1"], d["trigger_f1"]), row["fold_min_aet"], d["event_f1"], -row["reason_rate"], -ckpt)


def event_arg_score(row):
    d = row["routed_delta_vs_direct"]
    ckpt = int(row["checkpoint"].split("-", 1)[1])
    return (d["event_f1"] + d["argument_f1"], min(d["argument_f1"], d["event_f1"]), d["trigger_f1"], row["fold_min_aet"], -row["reason_rate"], -ckpt)
...
def render_table(rows):
    ...
    for row in rows:
        ...
        lines.append(
            "| {name} | {rate:.1%} | d/l/m/f={d}/{l}/{m}/{f} | {delta} | {floor:+.4f} | {a:.4f} / {e:.4f} / {t:.4f} |".format(
                name=row["name"],
...
def render_report(payload):
    lines = [
        "# Multibudget Four-Class Router M09 Dev Sweep",
        "",
...
def main():
    ...
    rows = sweep(direct_rows, light_rows, mid_rows, full_rows, common_keys)
    all_nonnegative = [row for row in rows if is_all_nonnegative(row)]
    if not all_nonnegative:
        raise RuntimeError("no all-nonnegative M09 dev candidate")
