...
def generator_prompt(row: dict) -> str:
    ...
    payload = {
        "task": "Generate general target-style-calibrated natural-language CoT supervision for event extraction.",
        "goal": (
            ...
            "The central goal is to teach the extractor how to scan candidate mentions, choose minimal triggers, "
            "ground roles locally, suppress unsupported extra events, and separate trigger text from evidence."
        ),
        "important_positioning": [
            ...
            "Do not mention model names, model errors, Direct, E40, E45, E46, or case labels in the output.",
        ...
        "thinking_strategy": [
            ...
            "First enumerate candidate event mentions in the text at a high level before finalizing events, including later or less salient mentions when the sentence contains several schema-compatible cues.",
            "For each target event, ground the event type in the provided schema cards and briefly contrast close or ambiguous types when a trigger could support multiple schemas.",
            "Explain minimal trigger normalization: trigger.text is the shortest lexical anchor copied from the target event, while trigger.evidence may be a larger local phrase or clause containing that trigger.",
            "Explicitly avoid putting auxiliaries, aspectual verbs, reporting words, diagnosis words, or surrounding context into trigger.text unless the target trigger itself contains them.",
            "Explain argument-role grounding: include only arguments explicitly supported by local evidence and state how each argument participates in the event role near the trigger.",
            "Apply role abstention: do not fill a role from discourse background, causal context, document topic, or plausible world knowledge when the local event mention does not express that role.",
            "Explain extraction-style control: avoid duplicate events, reject semantically plausible but weakly grounded extra events, and do not stop after the first plausible mention if another target-style event mention is present.",
            "End by verifying that every final trigger and argument has short contiguous local evidence from the text, and that trigger.text is distinct from the evidence phrase when the evidence is wider.",
        ],
        "thinking_quality_requirements": [
            ...
            "It must include candidate-mention scanning or coverage reasoning, not only explain the already selected final event.",
            "It must describe the minimal trigger boundary and the larger evidence phrase separately.",
            "It must mention at least one reason for excluding nearby context, extra candidates, or unsupported roles when the text contains potentially confusing context.",
            "It must describe why the evidence phrase is local and sufficient for the trigger or argument relation.",
        ],
        "final_json_schema": {
            "events": [
                {
...
def verifier_prompt(row: dict, thinking: str, final_obj: dict) -> str:
    ...
    payload = {
        ...
        "pass_requirements": [
            ...
            "The reasoning explicitly includes candidate-mention coverage before final event selection.",
...
def make_evidence_row(row: dict, thinking: str, final_obj: dict, dataset_role: str, run_name: str) -> dict:
    ...
    out["instruction"] = (
        ...
        "First output `<thinking>...</thinking>` with target-style-calibrated natural-language reasoning: scan candidate event mentions, "
        "ground event meanings in the schema, contrast close types when needed, choose minimal textual triggers, keep wider evidence separate from trigger text, "
        "ground arguments in local evidence, abstain from unsupported roles, suppress semantically plausible but weakly grounded extra events, and verify the final answer. "
    ...
    meta.update(
        {
            "adaptive_source": "target_style_evidence_cot_e46",
            "adaptive_target_style": "target_style_calibrated_thinking_surface_evidence_cot",
            "e46_run_name": run_name,
            "e46_generator_model": "deepseek-v4-pro",
            "e46_verifier_model": "deepseek-v4-pro",
        }
    )
    return out
...
def make_eval_evidence_row(row: dict, dataset_role: str, run_name: str) -> dict:
    placeholder = (
        "Scan candidate event mentions, ground event meanings in the schema, contrast close types, choose minimal textual triggers, "
        "keep wider evidence separate from trigger text, ground arguments in local evidence, abstain from unsupported roles, "
        "suppress weakly grounded extra events, and keep the final answer surface-only without numeric offsets."
    )
...
def parse_args():
    ...
    parser.add_argument("--run_name", default="e46b_qwen4_seed1500_target_style_cot")
...
def main():
    ...
    if args.output_dir is None:
        args.output_dir = REPO / "outputs/stage2_strategy_cot_e46" / f"{args.run_name}_20260605"
    ...
    summary["mode"] = "target_style_evidence_cot_e46"
    summary["qwen4_warm_start"] = QWEN4_WARM_START
    e40.write_json(args.output_dir / "e46_summary.json", summary)
