...
LOG_ROOT="outputs/stage2_1_7b_standard_format_ablation/e23_logs_20260526"
DEVPICK_ROOT="outputs/stage2_1_7b_standard_format_ablation/e23_devpick_20260526"
FORMAL_ROOT="outputs/stage2_1_7b_standard_format_ablation/e23_formal_20260526"
...
branch_for() {
  case "$1" in
    e23a) echo "eventmentions_budget_e23a_argument_table" ;;
    e23b) echo "eventmentions_budget_e23b_schema_grounded_role_table" ;;
    e23c) echo "eventmentions_budget_e23c_event_by_event_role_table" ;;
...
budgets_for() {
  case "$1" in
    e23a|e23b|e23c) echo "none light standard deep" ;;
    *) echo "unknown variant: $1" >&2; exit 2 ;;
  esac
}
...
train_one() {
  local variant="$1"
  local gpu="$2"
  local name="stage2_1_7b_${variant}_standard_format_ablation_train_20260526"
...
devpick_one() {
  local variant="$1"
  local gpu="$2"
  local name="stage2_1_7b_${variant}_standard_format_ablation_devpick_20260526"
  if docker ps -a --format '{{.Names}}' | grep -Fxq "${name}"; then
    echo "container already exists: ${name}" >&2
    return 0
  fi
...
eval_one() {
  ...
  local name="stage2_1_7b_${variant}_standard_format_ablation_forced_${budget}_${split}_20260526"
