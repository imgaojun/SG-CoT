...
LOG_ROOT="outputs/stage2_1_7b_step_decomposed_reasoning/e26_logs_20260527"
DEVPICK_ROOT="outputs/stage2_1_7b_step_decomposed_reasoning/e26_devpick_20260527"
FORMAL_ROOT="outputs/stage2_1_7b_step_decomposed_reasoning/e26_formal_20260527"
...
branch_for() {
  case "$1" in
    e26a) echo "eventmentions_budget_e26a_mention_schema" ;;
    e26b) echo "eventmentions_budget_e26b_mention_schema_groundlite" ;;
    e26c) echo "eventmentions_budget_e26c_mention_schema_verify" ;;
...
budgets_for() {
  case "$1" in
    e26a|e26b|e26c) echo "none light standard deep" ;;
    *) echo "unknown variant: $1" >&2; exit 2 ;;
  esac
}
...
selected_adapter() {
  local variant="$1"
  local summary="${DEVPICK_ROOT}/${variant}/forced_standard_dev/selection_summary.json"
  local fallback
...
train_one() {
  local variant="$1"
  local gpu="$2"
  local name="stage2_1_7b_${variant}_step_reasoning_train_20260527"
...
devpick_one() {
  local variant="$1"
  local gpu="$2"
  local name="stage2_1_7b_${variant}_step_reasoning_devpick_20260527"
  if docker ps -a --format '{{.Names}}' | grep -Fxq "${name}"; then
    echo "container already exists: ${name}" >&2
    return 0
  fi
  docker_common --name "${name}" --gpus "\"device=${gpu}\"" "${IMAGE}" bash -lc "
    set -euo pipefail
    mkdir -p ${LOG_ROOT}/${variant} ${DEVPICK_ROOT}/${variant}/forced_standard_dev
    ckpts=\$(find $(run_dir "${variant}") -maxdepth 1 -type d -name 'checkpoint-*' -printf '%f\n' | sort -V | tr '\n' ' ')
    python ${SHORTLIST_SCRIPT} \
      --base_model ${BASE_MODEL} \
      --run_dir $(run_dir "${variant}") \
      --eval_jsonl $(data_prefix "${variant}")_forced_standard_dev_seen_pos.jsonl \
      --output_root ${DEVPICK_ROOT}/${variant}/forced_standard_dev \
      ...
      --status_json ${DEVPICK_ROOT}/${variant}/forced_standard_dev/status.json \
      --reuse_existing
    HOST_UGID=\$(stat -c '%u:%g' /workspace/project)
...
eval_one() {
  ...
  local name="stage2_1_7b_${variant}_step_reasoning_forced_${budget}_${split}_20260527"
...
case "${1:-}" in
  train) train_one "${2:-}" "${3:-2}" ;;
  devpick) devpick_one "${2:-}" "${3:-2}" ;;
