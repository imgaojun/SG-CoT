...
LOG_ROOT="outputs/stage2_4b_reason_format_ablation/e15_logs_20260522"
DEVPICK_ROOT="outputs/stage2_4b_reason_format_ablation/e15_devpick_20260522"
FORMAL_ROOT="outputs/stage2_4b_reason_format_ablation/e15_formal_20260522"
...
branch_for() {
  case "$1" in
    ...
    e15b_minimaltype) echo "confrare10_typeonlylite_reasonfmt_e15b_minimaltype" ;;
...
train_one() {
  ...
  name="stage2_4b_e15_${variant}_train_20260522"
...
devpick_one() {
  ...
  name="stage2_4b_e15_${variant}_devpick_20260522"
  if docker ps -a --format '{{.Names}}' | grep -Fxq "${name}"; then
    echo "container already exists: ${name}" >&2
    return 0
  fi
...
eval_one() {
  ...
  name="stage2_4b_e15_${variant}_${mode}_${split}_20260522"
...
case "${1:-}" in
  train)
    ...
    ;;
  train-all)
    train_one e15a_noreasonblock "${2:-0}"
    train_one e15b_minimaltype "${3:-1}"
    train_one e15c_finalfirst "${4:-2}"
    ;;
  devpick)
    devpick_one "${2:?variant}" "${3:-0}"
    ;;
  devpick-all)
    devpick_one e15a_noreasonblock "${2:-0}"
    devpick_one e15b_minimaltype "${3:-1}"
    devpick_one e15c_finalfirst "${4:-2}"
    ;;
  formal)
