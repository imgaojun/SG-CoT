...
LOG_ROOT="outputs/stage2_4b_strong_reason_supervision/e18_logs_20260523"
DEVPICK_ROOT="outputs/stage2_4b_strong_reason_supervision/e18_devpick_20260523"
FORMAL_ROOT="outputs/stage2_4b_strong_reason_supervision/e18_formal_20260523"
...
train_one() {
  local gpu="$1"
  local name="stage2_4b_e18a_latentreason_outcomepos_train_20260523"
...
devpick_one() {
  local gpu="$1"
  local name="stage2_4b_e18a_latentreason_outcomepos_devpick_20260523"
  if docker ps -a --format '{{.Names}}' | grep -Fxq "${name}"; then
    echo "container already exists: ${name}" >&2
    return 0
  fi
...
eval_one() {
  ...
  local name="stage2_4b_e18a_latentreason_outcomepos_${mode}_${split}_20260523"
...
case "${1:-}" in
  train) train_one "${2:-0}" ;;
  devpick) devpick_one "${2:-0}" ;;
  formal)
    eval_one forced_direct test_seen "${2:-0}"
    eval_one forced_reason test_seen "${3:-1}"
    eval_one forced_direct test_unseen "${4:-2}"
    eval_one forced_reason test_unseen "${5:-3}"
    ;;
  *)
