...
CKPT="checkpoint-1939"
ADAPTER="outputs/stage2_adaptive_runs_user/${RUN_PREFIX}_${BRANCH}_full/${CKPT}"
FORMAL_ROOT="outputs/stage2_adaptive_runs_user_formal_nll_gated/richere_split1_qwen3_1_7b_adaptive_${BRANCH}/${CKPT}"
ROUTE_SCORE_ROOT="outputs/stage2_adaptive_route_likelihood_probe/sharedbase_routeauxclf1x_formal_ckpt1939_20260515"
...
launch_extraction() {
  ...
  local mode="$2"
  local host_gpu="$3"
  local name="adaptive_sharedbase_nllformal_${CKPT}_${mode}_${split}_20260515"
  ...
  local log="${LOG_DIR}/adaptive_sharedbase_nllformal_${CKPT}_${mode}_${split}_20260515.log"
...
launch_score() {
  local split="$1"
  local host_gpu="$2"
  local name="adaptive_sharedbase_nllformal_${CKPT}_route_nll_${split}_20260515"
  local eval_jsonl="data/stage2_adaptive_datasets/${DATA_PREFIX}_outcome15cal_nlltop15_routecls_scorebase_${split}_pos.jsonl"
  local output_dir="${ROUTE_SCORE_ROOT}/${split}"
  local log="${LOG_DIR}/adaptive_sharedbase_nllformal_${CKPT}_route_nll_${split}_20260515.log"
...
summarize_split() {
  ...
  python3 src/stage2_analysis/analyze_adaptive_outcome_router_execution.py \
    ...
    --output_json "reports/artifacts/2026-05-15_stage2_adaptive_sharedbase_route_nll_formal_${CKPT}_${split}.json" \
    --output_md "reports/2026-05-15_stage2_adaptive_sharedbase_route_nll_formal_${CKPT}_${split}.md"
}
...
case "${1:-}" in
  launch)
    ...
    launch_extraction test_seen forced_reason 0
    launch_extraction test_unseen forced_reason 1
    launch_score test 2
    launch_score test_seen 3
