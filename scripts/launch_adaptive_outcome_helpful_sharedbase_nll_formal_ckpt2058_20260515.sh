...
FORMAL_ROOT="outputs/stage2_adaptive_runs_user_formal_nll_gated/outcome_helpful_sharedbase_20260515/richere_split1_qwen3_1_7b_adaptive_${BRANCH}/${CKPT}"
ROUTE_SCORE_ROOT="outputs/stage2_adaptive_route_likelihood_probe/outcome_helpful_sharedbase_formal_20260515/${BRANCH}/${CKPT}"
...
launch_extraction() {
  ...
  local mode="$2"
  local host_gpu="$3"
  local name="adaptive_outcome_helpful_sharedbase_nllformal_${CKPT}_${mode}_${split}_20260515"
  ...
  local log="${LOG_DIR}/adaptive_outcome_helpful_sharedbase_nllformal_${CKPT}_${mode}_${split}_20260515.log"
...
launch_score() {
  local split="$1"
  local host_gpu="$2"
  local name="adaptive_outcome_helpful_sharedbase_nllformal_${CKPT}_route_nll_${split}_20260515"
  ...
  local log="${LOG_DIR}/adaptive_outcome_helpful_sharedbase_nllformal_${CKPT}_route_nll_${split}_20260515.log"
...
summarize_split() {
  ...
  python3 src/stage2_analysis/analyze_adaptive_outcome_router_execution.py \
    ...
    --output_json "reports/artifacts/2026-05-15_stage2_adaptive_outcome_helpful_sharedbase_nll_formal_${CKPT}_${split}.json" \
    --output_md "reports/2026-05-15_stage2_adaptive_outcome_helpful_sharedbase_nll_formal_${CKPT}_${split}.md"
}
...
case "${1:-}" in
  launch)
    ...
    launch_extraction test_unseen forced_reason 6
    launch_score test 7
    launch_score test_seen 0
    launch_score test_unseen 1
    ;;
  summarize)
