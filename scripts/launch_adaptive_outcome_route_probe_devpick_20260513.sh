...
launch_branch() {
  local branch="$1"
  local host_gpu="$2"
  local name="adaptive_outcome_${branch}_route_devpick_20260513"
  local run_dir="${RUN_PREFIX}_${branch}_full"
  local eval_jsonl="${DATA_PREFIX}_${branch}_dev_seen_pos.jsonl"
  local output_root="${PICK_PREFIX}_${branch}_full_route_dev_seen_max16"
  local log="${LOG_PREFIX}_${branch}_full_route_devpick_max16.log"
