...
OUT_BASE="outputs/stage2_strategy_cot_e37/formal_e37_seed500_20260604"
LOG_BASE="${OUT_BASE}/logs"
ADAPTER="${RUN_BASE}_e37_seed500_thinking_natural_cot_full/checkpoint-75"
...
launch_eval() {
  ...
  docker_common --name "${name}" --gpus "device=${gpu}" "${IMAGE}" bash -lc "
    ...
    python '${EVAL_SCRIPT}' \
      ...
      --eval_jsonl '${DATA_BASE}_e37_seed500_thinking_natural_cot_${split}_pos.jsonl' \
...
case "${1:-all}" in
  all)
    mkdir -p "${LOG_BASE}"
    launch_eval stage2_1_7b_e37_seed500_thinking_eval_seen_20260604 1 test_seen
    launch_eval stage2_1_7b_e37_seed500_thinking_eval_unseen_20260604 2 test_unseen
    ;;
  *)
    echo "usage: $0 [all]" >&2
    exit 2
