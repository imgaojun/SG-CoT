...
BRANCH="e45_qwen4_seed1500_task_grounded_cot_thinking_evidence_cot"
...
OUT_BASE="outputs/stage2_strategy_cot_e45/qwen4_task_grounded_cot_20260605"
...
train() {
  local gpu="${1:-1}"
  local name="stage2_e45_qwen4_task_grounded_cot_train_20260605"
  local log_path="${LOG_BASE}/qwen4_task_grounded_cot_train.log"
...
eval_pair() {
  ...
  local name="stage2_e45_qwen4_task_grounded_cot_ckpt${ckpt}_eval_pair_20260605"
  docker rm -f "${name}" >/dev/null 2>&1 || true
  docker_common --name "${name}" --gpus "device=${gpu}" "${IMAGE}" bash -lc "
    set -euo pipefail
...
case "${1:-}" in
  ...
  eval)
    mkdir -p "${LOG_BASE}"
    eval_pair "${2:?checkpoint}" "${3:-1}"
    ;;
  *)
    echo "usage: $0 train [gpu] | eval <ckpt> [gpu]" >&2
    exit 2
    ;;
esac
