...
BRANCH="e46b_qwen4_seed1500_target_style_cot_thinking_evidence_cot"
...
OUT_BASE="outputs/stage2_strategy_cot_e46/qwen4_target_style_cot_20260605"
...
train() {
  local gpu="${1:-1}"
  local name="stage2_e46_qwen4_target_style_cot_train_20260605"
  local log_path="${LOG_BASE}/qwen4_target_style_cot_train.log"
...
eval_pair() {
  ...
  local name="stage2_e46_qwen4_target_style_cot_ckpt${ckpt}_eval_pair_20260605"
  docker rm -f "${name}" >/dev/null 2>&1 || true
  docker_common --name "${name}" --gpus "device=${gpu}" "${IMAGE}" bash -lc "
    set -euo pipefail
