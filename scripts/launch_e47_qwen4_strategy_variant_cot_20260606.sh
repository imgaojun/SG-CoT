...
case "${VARIANT}" in
  e47a)
    BRANCH="e47a_qwen4_seed1500_abstention_first_cot_thinking_evidence_cot"
    LABEL="e47a_abstention_first"
    ;;
  e47b)
    BRANCH="e47b_qwen4_seed1500_candidate_audit_cot_thinking_evidence_cot"
    LABEL="e47b_candidate_audit"
    ;;
  e47c)
    BRANCH="e47c_qwen4_seed1500_compact_target_style_cot_thinking_evidence_cot"
    LABEL="e47c_compact_target_style"
    ;;
  e47c2)
    BRANCH="e47c2_qwen4_seed1500_compact_taganchored_cot_thinking_evidence_cot"
    LABEL="e47c2_compact_taganchored"
    ;;
  *)
...
OUT_BASE="outputs/stage2_strategy_cot_e47/${LABEL}_qwen4_20260606"
...
train() {
  local gpu="${1:-1}"
  local name="stage2_${LABEL}_qwen4_train_20260606"
...
eval_pair() {
  ...
  local name="stage2_${LABEL}_qwen4_ckpt${ckpt}_eval_pair_20260606"
  docker rm -f "${name}" >/dev/null 2>&1 || true
  docker_common --name "${name}" --gpus "device=${gpu}" "${IMAGE}" bash -lc "
    set -euo pipefail
...
case "${1:-}" in
  ...
  eval)
    ...
    ;;
  *)
    echo "usage: $0 train <e47a|e47b|e47c|e47c2> [gpu] | eval <e47a|e47b|e47c|e47c2> <ckpt> [gpu]" >&2
    exit 2
    ;;
esac
