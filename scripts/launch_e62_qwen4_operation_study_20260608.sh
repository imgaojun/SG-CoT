...
VARIANT="${2:-precision_plus}"
case "${VARIANT}" in
  full)
    BRANCH="e62a_full_glm51_seed1500_thinking_evidence_cot"
    LABEL="e62a_full"
    ;;
  no_close)
    BRANCH="e62a_no_close_glm51_seed1500_thinking_evidence_cot"
    LABEL="e62a_no_close"
    ;;
  no_role)
    BRANCH="e62a_no_role_glm51_seed1500_thinking_evidence_cot"
    LABEL="e62a_no_role"
    ;;
  no_suppress)
    BRANCH="e62a_no_suppress_glm51_seed1500_thinking_evidence_cot"
    LABEL="e62a_no_suppress"
    ;;
  precision_plus)
    BRANCH="e62b_precision_plus_glm51_seed1500_thinking_evidence_cot"
    LABEL="e62b_precision_plus"
    ;;
  *)
...
OUT_BASE="outputs/stage2_strategy_cot_e62/${LABEL}_qwen4_20260608"
...
train() {
  local gpu="${1:-1}"
  local name="stage2_${LABEL}_qwen4_train_20260608"
...
eval_pair() {
  ...
  local name="stage2_${LABEL}_qwen4_ckpt${ckpt}_eval_pair_20260608"
  docker rm -f "${name}" >/dev/null 2>&1 || true
  docker_common --name "${name}" --gpus "device=${gpu}" "${IMAGE}" bash -lc "
    set -euo pipefail
...
case "${1:-}" in
  ...
  eval)
    ...
    ;;
  *)
    echo "usage: $0 train <full|no_close|no_role|no_suppress|precision_plus> [gpu] | eval <variant> <ckpt> [gpu]" >&2
    exit 2
    ;;
esac
