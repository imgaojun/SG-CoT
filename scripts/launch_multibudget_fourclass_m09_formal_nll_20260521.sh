...
launch_score() {
  ...
  local split="$2"
  local host_gpu="$3"
  local name="multibudget_m09_formal_nll_${ckpt}_${split}_20260521"
...
case "${1:-}" in
  launch)
    launch_score checkpoint-400 test "${2:-0}"
    launch_score checkpoint-400 test_seen "${3:-1}"
    launch_score checkpoint-400 test_unseen "${4:-2}"
    launch_score checkpoint-350 test "${5:-3}"
    launch_score checkpoint-350 test_seen "${6:-4}"
    launch_score checkpoint-350 test_unseen "${7:-7}"
    ;;
  launch_one)
    if [[ $# -lt 2 ]]; then
    ...
    ;;
  *)
    echo "usage: $0 launch [gpu400_test] [gpu400_seen] [gpu400_unseen] [gpu350_test] [gpu350_seen] [gpu350_unseen]" >&2
