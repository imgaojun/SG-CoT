...
SCORE_ROOT="outputs/stage2_multibudget/formal_route_likelihood_20260521/${BRANCH}"
...
launch_score() {
  ...
  local split="$2"
  local host_gpu="$3"
  local name="multibudget_m08_formal_nll_${ckpt}_${split}_20260521"
  ...
  docker run -d \
    ...
    bash -lc "
      ...
      python src/stage2_quality_validation/score_adaptive_multiroute_choice_likelihood.py \
...
case "${1:-}" in
  launch)
    # Dev-locked candidates: checkpoint-700 balanced and checkpoint-400 event/argument.
    launch_score checkpoint-700 test "${2:-0}"
    launch_score checkpoint-700 test_seen "${3:-1}"
    launch_score checkpoint-700 test_unseen "${4:-2}"
    launch_score checkpoint-400 test "${5:-3}"
    launch_score checkpoint-400 test_seen "${6:-4}"
    launch_score checkpoint-400 test_unseen "${7:-7}"
    ;;
  launch_one)
    if [[ $# -lt 2 ]]; then
      echo "usage: $0 launch_one <checkpoint> [gpu_test] [gpu_seen] [gpu_unseen]" >&2
      exit 2
    fi
    launch_score "$2" test "${3:-0}"
    launch_score "$2" test_seen "${4:-1}"
    launch_score "$2" test_unseen "${5:-2}"
    ;;
  *)
    echo "usage: $0 launch [gpu700_test] [gpu700_seen] [gpu700_unseen] [gpu400_test] [gpu400_seen] [gpu400_unseen]" >&2
    echo "       $0 launch_one <checkpoint> [gpu_test] [gpu_seen] [gpu_unseen]" >&2
    exit 2
    ;;
esac
