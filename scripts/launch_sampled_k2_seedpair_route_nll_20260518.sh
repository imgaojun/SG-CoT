...
EXPERIMENT_ID="${EXPERIMENT_ID:-sampled_k2_seedpair_route_nll_margin_calibration_20260518}"
CHECKPOINTS="${CHECKPOINTS:-checkpoint-25 checkpoint-50 checkpoint-75 checkpoint-100}"
OUTPUT_ROOT="${OUTPUT_ROOT:-outputs/stage2_adaptive_route_seedpair_nll_20260518/${BRANCH}}"
  ...
  bash -lc "
    ...
    for ckpt in ${CHECKPOINTS}; do
      ...
      for pair in 17_18 19_20 21_22 23_24; do
        ...
        if [[ -s \"\${out_dir}/scores.jsonl\" ]]; then
          echo \"skip existing \${ckpt} seedpair\${pair}\" | tee -a ${LOG}
          continue
        fi
        mkdir -p \"\${out_dir}\"
        echo \"[seedpair-route-nll] \${ckpt} seedpair\${pair}\" | tee -a ${LOG}
