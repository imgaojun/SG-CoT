...
BRANCH = "eventmentions_budget_e22a_trigger_preserving_forms"
        ...
        out.append(
            {
                ...
                "arguments": args,
                "checks": checks + ["trigger_span_preserved"] if trigger else checks,
            }
        )
    return {"events": out}
...
def budget_instruction(budget):
    block = {
        ...
        "standard": "After the budget tag, output `<ROLE_TABLE>{...}</ROLE_TABLE>` with event_type, trigger text/start/end, and filled roles.",
...
def clone(row, budget, role):
    ...
    meta.update(
        {
            "adaptive_source": "trigger_preserving_forms_e22",
            "adaptive_target_style": "trigger_preserving_reason_form_budget",
            "adaptive_dataset_role": role,
            "adaptive_reasoning_budget": budget,
            "adaptive_budget_label": budget,
            "e22_branch": BRANCH,
            "e22_budget": budget,
            "e22_event_count": stats["event_count"],
            "e22_argument_count": stats["argument_count"],
            "e22_role_count": stats["role_count"],
        }
    )
    return out
...
def write_note(train_name, dev_name, audit):
    ...
    body = f"""---
id: {exp_id}
title: Stage2 1.7B E22A Trigger-Preserving Reasoning Forms
kind: experiment
status: planned
...
tags:
  ...
  - trigger-preserving
  - explicit-reasoning
objective: Test whether trigger-preserving intermediate forms keep E21A standard/deep Argument/Event gains while reducing Trigger loss.
artifacts:
  configs:
    - {config_path}
  outputs:
...
# Stage2 1.7B E22A Trigger-Preserving Reasoning Forms

## Goal

Train one model with four visible budget-specific forms. Compared with E21A, standard/deep intermediate blocks preserve trigger text/start/end.
...
```bash
cd {REPO}
python3 scripts/prepare_1_7b_trigger_preserving_forms_e22_20260526.py
bash scripts/launch_1_7b_trigger_preserving_forms_e22_20260526.sh train e22a 0
bash scripts/launch_1_7b_trigger_preserving_forms_e22_20260526.sh devpick e22a 0
bash scripts/launch_1_7b_trigger_preserving_forms_e22_20260526.sh formal e22a 0 1 2 3
```
...
def main():
    ...
    audit = {
        "recipe": "trigger-preserving explicit reasoning forms for none/light/standard/deep",
