...
EVAL_BRANCH = "confrare10_typeonlylite_reasonfmt_e18a_latentreason_outcomepos"
...
def latent_reason_instruction():
    return (
        ...
        "Use the reasoning route internally: identify the expressed event type first, verify trigger spans against the token list, "
        "then ground each argument role in the schema. Do not output the reasoning. "
...
def clone(row, source, duplicate_idx=0):
    ...
    meta["e18b_source"] = source
    meta["e18b_duplicate_index"] = duplicate_idx
    meta["e18b_branch"] = BRANCH
    meta["adaptive_source"] = "event_preserving_reason_e18b"
    return out
...
def reason_row(source_row, gold_row, gains, duplicate_idx=0):
    out = clone(source_row, "event_positive_reason", duplicate_idx)
    out["instruction"] = latent_reason_instruction()
    ...
    meta.update(
        {
            "adaptive_dataset_role": "train",
            "adaptive_route_mode": "forced_reason",
            "adaptive_route_label": "reason",
            "adaptive_target_style": "latent_no_reason_block_event_preserving",
            "e18b_outcome_gain_argument_f1": gains.get("argument_f1", 0.0),
            "e18b_outcome_gain_event_f1": gains.get("event_f1", 0.0),
            "e18b_outcome_gain_trigger_f1": gains.get("trigger_f1", 0.0),
            "e18b_outcome_gain_sum": gains.get("sum", 0.0),
        }
    )
    return out
...
def build_train_rows():
    ...
    for k, gains in positives.items():
        ...
        for dup in range(REASON_REPEAT):
            reason_part.append(reason_row(source_by_key[k], formal_by_key[k], gains, dup))
    ...
    audit = {
        ...
        "selection_rule": "reason_gain event_f1 > 0 and summed argument/event/trigger gain > 0 on train outcome mining",
        "adaptive_train_count": len(adaptive),
        "outcome_positive_count": len(positives),
        "missing_positive_keys": missing,
...
def write_note(train_name, dev_name, audit):
    timestamp = now_iso()
    exp_id = "2026-05-23_stage2_4b_event_preserving_reason_e18b_latentreason_eventpos_sumpos_r5_richere_split1_oracle_mixed_noise_qwen3_4b"
    ...
    body = f"""---
id: {exp_id}
title: Stage2 4B Event Preserving Reason E18B Latent Reason Event Positives R5
kind: experiment
status: planned
...
tags:
  ...
  - outcome-supervision
  - event-preserving
objective: Train an Event-preserving latent reason expert by selecting train examples where reasoning improves Event and total A/E/T outcome.
artifacts:
  configs:
    - {config_path}
  outputs:
...
# Stage2 4B Event Preserving Reason E18B

## Goal

Test whether an Event-focused outcome-positive recipe can keep E18A's Argument gain while recovering Event.
...
```bash
cd {REPO}
python3 scripts/prepare_4b_event_preserving_reason_e18b_20260523.py
bash scripts/launch_4b_event_preserving_reason_e18b_20260523.sh train 1
bash scripts/launch_4b_event_preserving_reason_e18b_20260523.sh devpick 1
bash scripts/launch_4b_event_preserving_reason_e18b_20260523.sh formal 1 2 3 4
```
...
def main():
    ...
    dev_rows = e15.load_jsonl(DATA_DIR / f"{ADAPTIVE_PREFIX}_{EVAL_BRANCH}_dev_seen_pos.jsonl")
    for row in dev_rows:
        row["instruction"] = free_route_instruction()
        row.setdefault("meta", {})["adaptive_source"] = "event_preserving_reason_e18b"
        row.setdefault("meta", {})["adaptive_target_style"] = "latent_no_reason_block_event_preserving"
    write_dataset(dev_name, dev_rows)

    eval_names = []
    for split in ["test_seen", "test_unseen"]:
        for mode in ["forced_direct", "forced_reason"]:
            source_name = f"{ADAPTIVE_PREFIX}_{EVAL_BRANCH}_{mode}_{split}_pos"
