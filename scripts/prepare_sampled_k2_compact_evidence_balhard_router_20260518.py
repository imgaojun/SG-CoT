...
from scripts.prepare_sampled_evidence_balhard_router_20260518 import fmt  # noqa: E402


BRANCH = "sampled_k2_ckpt258_evidcompact_balhard_routecls_noauxwarm_lr2e6_save25"
SAMPLE_COUNT = 2
EVIDENCE_STYLE = "compact_v1_k2"
...
def render_compact_evidence(features: dict):
    lines = [
        "Compact repeated-output evidence (K=2 per route; no gold metrics):",
        (
...
def build_route_row(source_row, label_row, features, split: str, duplicate_index: int = 0):
    ...
    meta.update(
        {
            "adaptive_source": "sampled_k2_compact_evidence_balhard_routecls",
            "adaptive_dataset_role": split,
            "adaptive_route_mode": "free_route",
            "adaptive_route_label": label_row["route_label"],
            "adaptive_target_style": "route_classifier_only_with_k2_compact_output_consistency_evidence",
...
def make_note(cfg_path: Path, output_dir: Path, train_meta: dict, dev_meta: dict, timestamp: str):
    exp_id = "2026-05-18_stage2_sampled_k2_compact_evidence_balhard_routecls_checkpoint258_richere_split1_oracle_mixed_noise_qwen3_1_7b"
    note = EXPERIMENT_DIR / f"{exp_id}.md"
    title = "Stage2 Sampled K2 Compact-Evidence Balanced-Hard Route Classifier Checkpoint-258"
    body = f"""---
id: {exp_id}
title: {title}
kind: experiment
...
tags:
  ...
  - compact-evidence
  - cost-ablation
objective: Test whether low-cost K=2 output-consistency evidence retains the routing benefit of K=8 compact evidence.
artifacts:
  configs:
    - {cfg_path}
  outputs:
    - {output_dir}
  reports:
    - {REPO / 'reports/2026-05-18_stage2_sampled_k2_compact_evidence_balhard_routecls_checkpoint258_dev_probe.md'}
related:
  plans:
    - {REPO / 'PLANS.md'}
  experiments:
    - {REPO / 'experiments/2026-05-18_stage2_sampled_k8_compact_evidence_balhard_routecls_checkpoint258_richere_split1_oracle_mixed_noise_qwen3_1_7b.md'}
context:
  dataset: RichERE
  split: split1
  label_source: {LABEL_SOURCE}
...
K=8 compact evidence is the strongest dev-only mechanism so far, but it is expensive because it requires repeated direct/reason sampling. This experiment keeps the same K=8-derived stable_reason/stable_direct supervision labels and trains the same balanced-hard route classifier, while exposing only K=2 output-consistency evidence in the route prompt.
...
- evidence fields: validity, consensus averages, trigger/argument/type consensus, count stability, cross-route disagreement, and derived consistency cues computed from only `2` samples per route.
...
python3 scripts/prepare_sampled_k2_compact_evidence_balhard_router_20260518.py
bash scripts/launch_sampled_confident_router_train_20260518.sh {BRANCH}=<gpu>
bash scripts/run_sampled_k2_compact_evidence_balhard_router_after_train_20260518.sh
```
...
- created K=2 compact-evidence balanced-hard route classifier datasets.
- reused K=8 sampled utility labels as supervision targets.
...
- compare K=2 compact evidence against K=8 compact evidence.
"""
