...
NEGATIVE_UNIQUE_MULTIPLIER = 8
MIN_PASSING_SEED_PAIRS = 2
...
def clear_structural_direct(features: dict):
    return (
        ...
        or features["route_argument_disagreement"] > 1.0 - ARG_TEXT_JACCARD_MIN
    )
    ...
    meta.update(
        {
            ...
            "sampled_evidence_style": f"compact_v2_k2_structproxy_strict_{pair_slug(pair_name)}",
            ...
            "structural_proxy_min_passing_seed_pairs": MIN_PASSING_SEED_PAIRS,
...
def select_structural_train_examples(labels, features_by_pair):
    ...
    for label in labels:
        ...
        if utility_label == "stable_reason" and pass_count >= MIN_PASSING_SEED_PAIRS:
            positives.append((label, pair_pass))
        elif utility_label in {"stable_direct", "stable_reason"}:
            clear_count = sum(
            ...
            direct_candidates.append(
                {
                    ...
                    "failed_strict_pair_count": len(SEED_PAIRS) - pass_count,
                    "failed_stable_reason": utility_label == "stable_reason",
                }
            )
    ...
    hard_pool = [
        ...
        if item["failed_stable_reason"]
        or item["clear_count"] > 0
        or item["label"].get("mean_gain", 0.0) > 0.0
        or item["label"].get("p_win", 0.0) > 0.25
        or item["label"].get("p_trigger_noharm", 1.0) < 0.75
        or item["label"].get("direct_mean_score", 2.25) < 1.0
    ]
    hard_pool = sorted(
        hard_pool,
        key=lambda item: (
            item["failed_stable_reason"],
            item["failed_strict_pair_count"],
            item["clear_count"],
            rank_hard_negative(item["label"]),
        ),
        reverse=True,
    )
    ...
    for item in hard_pool:
        ...
        if len(negatives) >= target_negatives:
            break
    if len(negatives) < target_negatives:
        rest = [item["label"] for item in direct_candidates if item["label"]["wnd_id"] not in seen]
    ...
    return positives, negatives[:target_negatives], {
        ...
        "stable_direct_count": sum(1 for item in direct_candidates if item["label"].get("utility_label") == "stable_direct"),
        "failed_stable_reason_negative_candidate_count": sum(1 for item in direct_candidates if item["failed_stable_reason"]),
        ...
        "min_passing_seed_pairs": MIN_PASSING_SEED_PAIRS,
        "positive_rule": "stable_reason and strict structural pass on at least 2/4 K2 seed pairs",
        "negative_rule": "stable_direct plus failed-stable-reason hard false positives, prioritized by strict structural failures",
    }
...
def build_train_split(dataset_name: str):
    ...
    meta = {
        ...
        "negative_strategy": "hard_direct_plus_failed_reason_structural_false_positive_priority",
...
def build_dev_split(dataset_name: str):
    ...
    for pair_name, seeds in SEED_PAIRS:
        ...
        for source_row in source_rows:
            ...
            pair_pass = {
                current_pair: structural_pass(features_by_pair[current_pair][key])
                for current_pair, _current_seeds in SEED_PAIRS
            }
            route_label = (
                "reason"
                if label_row.get("utility_label") == "stable_reason"
                and sum(pair_pass.values()) >= MIN_PASSING_SEED_PAIRS
                and pair_pass[pair_name]
                else "direct"
            )
    ...
    selection_summary = {
        ...
        "min_passing_seed_pairs": MIN_PASSING_SEED_PAIRS,
        "positive_rule": "stable_reason with strict structural pass on at least 2/4 seed pairs, labeled reason only on passing pairs",
...
def make_note(cfg_path: Path, output_dir: Path, train_meta: dict, dev_meta: dict, timestamp: str):
    exp_id = "2026-05-19_stage2_sampled_k2_structural_proxy_strictv2_supervised_router_checkpoint258_richere_split1_oracle_mixed_noise_qwen3_1_7b"
    note = EXPERIMENT_DIR / f"{exp_id}.md"
    title = "Stage2 Sampled K2 Strict Structural-Proxy v2 Route Classifier Checkpoint-258"
    body = f"""---
id: {exp_id}
title: {title}
kind: experiment
...
tags:
  ...
  - compact-evidence
objective: Train a stricter route-choice selector with high-precision structural positives and hard failed-reason negatives after the v1 selector showed seen false positives.
artifacts:
  configs:
    - {cfg_path}
  outputs:
  ...
  reports:
    - {REPO / 'reports/2026-05-19_stage2_sampled_k2_structural_proxy_strictv2_router_dev_probe.md'}
related:
  plans:
    - {REPO / 'PLANS.md'}
  experiments:
...
Train a second selector that keeps the supervised route-choice setup but makes the labels higher precision. The v1 coverage-first selector learned useful ranking signal, but formal single-generation evaluation showed seen false positives. This variant treats failed stable-reason structural cases as hard direct negatives.
...
and strict structural pass on at least 2/4 K2 seed pairs
```

Structural pass uses train/dev available gold-free evidence:

```text
route_argument_disagreement <= 0.60
and reason_minus_direct_event_count_mean <= 0.0
```

Hard direct negatives include stable-direct cases and stable-reason cases that fail the strict structural requirement.

## Commands

```bash
cd /mnt/disk/gaojun/research/progressive-ee
python3 scripts/prepare_sampled_k2_structural_proxy_router_v2_20260519.py
...
- created strict v2 structural-proxy supervised route-classifier datasets.
...
- if the dataset audit is viable, launch training.
- after training, run route-NLL formal evaluation against the locked structural proxy reference.
"""
...
def main():
    ...
    manifest = CONFIG_DIR / "sampledk2_structural_proxy_router_v2_20260519.json"
    write_json(
        manifest,
        {
            "id": "sampledk2_structural_proxy_router_v2_20260519",
