...
SMOKE_CONFIG="configs/generated/stage2_preference/e116a_e81_reference_atomic_dpo_smoke40_seed42.yaml"
SMOKE_MODEL="/workspace/project/outputs/stage2_preference_runs/e116a_e81_reference_atomic_dpo_smoke40_seed42"
HOST_SMOKE_MODEL="${PROJECT_ROOT}/outputs/stage2_preference_runs/e116a_e81_reference_atomic_dpo_smoke40_seed42"
...
docker_common() {
  docker run --rm --user root --ipc host --shm-size 16g \
    ...
    -e LLAMAFACTORY_REF_BATCH_SIZE=1 \
    -e PYTHONPATH=/workspace/project/scripts/compat/trl024_prepare_deepspeed \
    -w /workspace/project "$@"
}
...
case "${1:-}" in
  train-smoke)
    ...
    if [[ -f "${HOST_SMOKE_MODEL}/trainer_state.json" ]]; then
      echo "E116A already ran; refusing to reuse its output directory" >&2
      exit 2
    fi
    gpu="${2:-0}"
    run_gpu "${gpu}" bash -lc \
      "set -o pipefail; FORCE_TORCHRUN=1 llamafactory-cli train '${SMOKE_CONFIG}' 2>&1 | tee '${BUILD_OUTPUT}/e116a_reference_dpo_smoke40_retry1_train.log'"
    ;;
  score-post)
    require_frozen
  ...
  gate)
    require_frozen
    docker_common "${IMAGE}" python scripts/compare_e116_reference_dpo_gate_20260712.py \
      ...
      --output_json "${SMOKE_MODEL}/e116_reference_dpo_gate.json" \
