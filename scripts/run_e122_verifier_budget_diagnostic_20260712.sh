...
RUN_NAME="e122a_verifier4096_smoke20"
OUTPUT_DIR="/workspace/project/outputs/stage2_confirmation_e122/${RUN_NAME}"
HOST_OUTPUT_DIR="${PROJECT_ROOT}/outputs/stage2_confirmation_e122/${RUN_NAME}"
...
[[ ! -e "${HOST_OUTPUT_DIR}" ]] || {
  echo "refusing to reuse E122 output directory: ${HOST_OUTPUT_DIR}" >&2
  exit 11
}
...
docker run --rm --user root \
  ...
  --output_json "${OUTPUT_DIR}/e122a_gate.json" \
  --expected_rows 20 \
  --min_accepted 19 \
  --max_attempts 3 \
  --verify_max_tokens 4096 \
