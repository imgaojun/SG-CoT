...
RUN_NAME="e124a_glm51_selfverifier4096_smoke40"
PROTOCOL="e124a-glm51-selfverifier4096-disjoint-trainonly-smoke40"
...
SOURCE="/workspace/project/data/stage2_confirmation_e124/e124a_disjoint_train_smoke40.jsonl"
...
[[ ! -e "${HOST_OUTPUT_DIR}" ]] || {
  echo "refusing to reuse E124 output directory: ${HOST_OUTPUT_DIR}" >&2
  exit 11
}
...
docker run --rm --user root --ipc host --shm-size 16g \
  ...
  python scripts/generate_strategy_variants_cot_e47_20260606.py \
    ...
    --run_prefix e124a_glm51_selfverifier4096 \
...
docker run --rm --user root \
  ...
  --output_json "${OUTPUT_DIR}/e124a_gate.json" \
  --protocol "${PROTOCOL}" \
  --expected_rows 40 \
  --min_accepted 38 \
  --max_attempts 3 \
