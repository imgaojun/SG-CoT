...
FORMAL_ROOT = REPO / "outputs/stage2_1_7b_step_decomposed_reasoning/e26_formal_20260527"
DEVPICK_ROOT = REPO / "outputs/stage2_1_7b_step_decomposed_reasoning/e26_devpick_20260527"
OUT_JSON = REPO / "reports/artifacts/2026-05-27_stage2_1_7b_step_decomposed_reasoning_e26.json"
OUT_MD = REPO / "reports/2026-05-27_stage2_1_7b_step_decomposed_reasoning_e26.md"

VARIANTS = {
    "e26a": {"label": "mention_schema", "budgets": ["none", "light", "standard", "deep"]},
    "e26b": {"label": "mention_schema_groundlite", "budgets": ["none", "light", "standard", "deep"]},
    "e26c": {"label": "mention_schema_verify", "budgets": ["none", "light", "standard", "deep"]},
}
...
def expected_tags(variant, budget):
    ...
    if budget == "light":
        tags.append("SCHEMA_PLAN")
    elif budget == "deep":
        tags.append("VERIFY")
    elif budget == "standard":
        tags.append({"e26a": "SCHEMA_PLAN", "e26b": "GROUND_HINT", "e26c": "VERIFY"}[variant])
    return tags
...
def tag_rates(variant, budget, split):
    ...
    if not total:
        return {}
    all_tags = ["EVENT_MENTIONS", "REASONING_BUDGET", "SCHEMA_PLAN", "GROUND_HINT", "VERIFY", "FINAL"]
    rates = {}
    for tag in all_tags:
...
def render(payload):
    lines = [
        "# 1.7B Forced Step-Decomposed Reasoning E26",
        "",
        "This report summarizes step and transmission ablations against E21A standard.",
        "",
    ]
    ...
    if test_rows:
        lines.extend(
            [
                ...
                "",
                "| variant | step transmission | A/E/T | delta vs none A/E/T | delta vs E21A standard A/E/T | expected form |",
                "|---|---|---:|---:|---:|---:|",
            ]
        )
    ...
    lines.extend(
        [
            "## Reading",
            "",
            "- E26A tests whether schema/role planning alone explains E21A's gains.",
            "- E26B tests lightweight grounding without span/offset copy burden.",
            "- E26C tests whether compact verification protects Event full-frame quality.",
        ]
    )
    return "\n".join(lines) + "\n"
...
def main():
    ...
    payload = {
        ...
        "variants": VARIANTS,
        "baseline": BASELINES,
        "dev_selection": [item for item in (load_dev_selection("e26a"), load_dev_selection("e26b"), load_dev_selection("e26c")) if item],
        "results": rows,
