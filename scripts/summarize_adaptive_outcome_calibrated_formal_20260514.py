...
DEFAULT_MANIFEST = REPO / "configs/generated/stage2_adaptive/richere_qwen3_1_7b_adaptive_outcome_calibrated_execution_gate_formal_manifest.json"
DEFAULT_OUTPUT_JSON = REPO / "reports/artifacts/2026-05-14_stage2_adaptive_outcome_calibrated_execution_gate_formal_summary.json"
DEFAULT_OUTPUT_MD = REPO / "reports/2026-05-14_stage2_adaptive_outcome_calibrated_execution_gate_formal_summary.md"
...
def branch_from_tag(tag: str):
    return tag.removesuffix("__execution_gate")
...
def render_markdown(payload):
    lines = [
        "# Adaptive Outcome-Calibrated Execution-Gate Formal Summary",
        "",
        "## Status",
        "",
    ...
    if not payload["runs"]:
        lines.extend(["## Reading", "", "- No branch passed the dev execution gate, so formal evaluation was not launched."])
        return "\n".join(lines) + "\n"
    ...
    lines.extend(
        [
            ...
            "- Treat `forced_reason_minus_forced_direct` as the remaining reason-expert headroom for the selected checkpoint.",
            "- A useful paper-main result requires free-route gains on `test_unseen` without a large `test_seen` regression.",
        ]
    )
    return "\n".join(lines) + "\n"
