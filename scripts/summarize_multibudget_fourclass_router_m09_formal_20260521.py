...
OUT_JSON = REPO / "reports/artifacts/2026-05-21_stage2_multibudget_fourclass_router_m09_formal.json"
OUT_MD = REPO / "reports/2026-05-21_stage2_multibudget_fourclass_router_m09_formal.md"
...
POLICIES = [
    {
        "name": "m09_balanced_dev_locked",
        "checkpoint": "checkpoint-400",
        "start_pct": 0.325,
        "end_pct": 0.400,
        "dev_delta_aet": "+0.0118/+0.0102/+0.0118",
    },
    {
        "name": "m09_event_arg_dev_locked",
        "checkpoint": "checkpoint-350",
        "start_pct": 0.000,
        "end_pct": 0.225,
        "dev_delta_aet": "+0.0105/+0.0132/+0.0039",
    },
]
...
def evaluate(policy, split):
    ...
    for _, key, _ in ranked:
        ...
        light_metrics.append(row_metric(light_rows[key]))
    ...
    return {
        **policy,
        ...
        "reason_rate": sum(route_counts[r] for r in ROUTES) / len(ranked) if ranked else 0.0,
        "selected_mean_score_gain": sum(selected_gain) / len(selected_gain) if selected_gain else 0.0,
        "direct": direct_summary,
        "forced_light_all": summarize_metrics(light_metrics),
        "forced_mid_all": summarize_metrics(mid_metrics),
        "forced_full_all": summarize_metrics(full_metrics),
        "routed": routed,
...
def render(payload):
    lines = [
        "# Multibudget Four-Class Router M09 Formal Replay",
        "",
...
def main():
    ...
    payload = {"branch": BRANCH, "score_root": SCORE_ROOT.as_posix(), "policies": POLICIES, "results": results}
