...
from src.stage2_analysis.analyze_adaptive_outcome_router_execution import (
    analyze_router,
    load_prediction_map,
    write_json,
    write_text,
...
def pass_gate(row):
    ...
    rr = free.get("route_reason_rate", 0.0)
    json_ok = free.get("json_valid_rate", free.get("final_json_valid_rate", 0.0)) >= 0.99
    rate_ok = 0.05 <= rr <= 0.20
    gain_ok = delta["argument_f1"] >= 0.005 or (
        delta["event_f1"] >= 0.005 and delta["argument_f1"] >= -0.003
    )
    return json_ok and rate_ok and gain_ok
...
def selection_score(row):
    ...
    return (
        ...
        delta["argument_f1"],
        delta["event_f1"],
        -abs(row["free_summary"].get("route_reason_rate", 0.0) - 0.15),
    )
...
def make_selection_summary(branch: str, best, output_path: Path):
    ...
    payload = {
        ...
        "metric_keys": ["execution_gate_score"],
        ...
        "selection_protocol": "adaptive_execution_gate",
        "frontier_protocol": "execution_gate",
...
def render_markdown(branch: str, rows, best):
    lines = [
        f"# Adaptive Execution Gate: {branch}",
        "",
        "| checkpoint | gate | reason rate | free arg/event | direct arg/event | routed arg/event | delta arg/event | helpful P/R/F1 |",
        "|---|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for row in rows:
        free = row["free_summary"]
        direct = row["execution"]["direct"]
        routed = row["execution"]["routed"]
        delta = row["execution"]["routed_delta_vs_direct"]
        helpful = row["execution"]["route_vs_positive_reason_helpful"]
        lines.append(
            "| `{}` | {} | {:.4f} | {:.4f}/{:.4f} | {:.4f}/{:.4f} | {:.4f}/{:.4f} | {:+.4f}/{:+.4f} | {:.3f}/{:.3f}/{:.3f} |".format(
                row["checkpoint_tag"],
                "pass" if row["gate_pass"] else "fail",
                free.get("route_reason_rate", 0.0),
                free.get("argument_f1", 0.0),
                free.get("event_f1", 0.0),
                direct["argument_f1"],
                direct["event_f1"],
                routed["argument_f1"],
                routed["event_f1"],
    ...
    lines.extend(
        [
            ...
            f"- selection summary: `outputs/stage2_adaptive_runs_user_devpick_frontier/protocol_selections/{branch}__execution_gate/selection_summary.json`",
            "",
        ]
    )
    return "\n".join(lines)
...
def main():
    ...
    output_json = Path(args.output_json) if args.output_json else REPO / f"reports/artifacts/2026-05-14_stage2_adaptive_{args.branch}_execution_gate.json"
    output_md = Path(args.output_md) if args.output_md else REPO / f"reports/2026-05-14_stage2_adaptive_{args.branch}_execution_gate.md"
    selection_summary = (
        Path(args.selection_summary)
        if args.selection_summary
        else REPO / f"outputs/stage2_adaptive_runs_user_devpick_frontier/protocol_selections/{args.branch}__execution_gate/selection_summary.json"
    )
