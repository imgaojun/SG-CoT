...
def render_instruction():
    return (
        "Do event extraction with a compact contrastive decision trace. "
        ...
        "`decisions` is a compact list of accepted event decisions only. "
        "Each decision must contain `event_type`, `trigger`, `contrast_type`, and `reason_code`. "
        "`contrast_type` should name one confusable alternative candidate when useful, otherwise use an empty string. "
        "`reason_code` must be one of `family_boundary`, `role_signature`, `trigger_signature`, or `context_support`. "
...
def render_input(row, candidate_types, schema_by_type):
    ...
    return (
        ...
        "- Top-level keys must be `decisions` and `events`.\n"
        "- `decisions` contains one compact accepted decision per final event.\n"
        "- Each decision contains `event_type`, `trigger`, `contrast_type`, and `reason_code`.\n"
        f"- `reason_code` must be one of: {codes_text}.\n"
        "- Use an empty string for `contrast_type` when no strong contrast type is needed.\n"
        "- `events` is the final extracted event list.\n"
...
def observed_roles(event):
    return {arg["role"] for arg in event["arguments"]}
...
def schema_trigger_cues(schema_by_type, event_type):
    return normalize_cue_tokens(schema_by_type[event_type].get("trigger_cues", []))


def contrast_score(event, chosen_type, other_type, schema_by_type):
    chosen_roles = schema_roles(schema_by_type, chosen_type)
    chosen_cues = schema_trigger_cues(schema_by_type, chosen_type)
    other_roles = schema_roles(schema_by_type, other_type)
    other_cues = schema_trigger_cues(schema_by_type, other_type)
    obs_roles = observed_roles(event)
    obs_trigger = observed_trigger_tokens(event)
    same_family = 1.0 if event_family(chosen_type) == event_family(other_type) else 0.0
    observed_role_overlap = jaccard(obs_roles, other_roles)
    observed_cue_overlap = jaccard(obs_trigger, other_cues)
    schema_role_overlap = jaccard(chosen_roles, other_roles)
    schema_cue_overlap = jaccard(chosen_cues, other_cues)
    return same_family * 10.0 + observed_role_overlap * 4.0 + observed_cue_overlap * 2.0 + schema_role_overlap + schema_cue_overlap


def choose_contrast_type(event, chosen_type, candidate_types, schema_by_type):
    ranked = []
    for other_type in candidate_types:
        if other_type == chosen_type:
            continue
        score = contrast_score(event, chosen_type, other_type, schema_by_type)
        ranked.append((score, other_type))
    if not ranked:
        return ""
...
def reason_code_for_event(event, chosen_type, contrast_type, schema_by_type):
    chosen_role_overlap = jaccard(observed_roles(event), schema_roles(schema_by_type, chosen_type))
    chosen_cue_overlap = jaccard(observed_trigger_tokens(event), schema_trigger_cues(schema_by_type, chosen_type))

    if contrast_type and event_family(chosen_type) == event_family(contrast_type):
        return "family_boundary"
    if chosen_role_overlap > 0.0 and chosen_role_overlap >= chosen_cue_overlap:
        return "role_signature"
    if chosen_cue_overlap > 0.0:
        return "trigger_signature"
    return "context_support"


def decision_for_event(event, candidate_types, schema_by_type):
    contrast_type = choose_contrast_type(event, event["event_type"], candidate_types, schema_by_type)
    return {
        "event_type": event["event_type"],
        "trigger": normalize_trigger(event["trigger"]),
        "contrast_type": contrast_type,
        "reason_code": reason_code_for_event(event, event["event_type"], contrast_type, schema_by_type),
    }


def render_output(events, candidate_types, schema_by_type):
    if not events:
        return json.dumps({"decisions": [], "events": []}, ensure_ascii=False)

    decisions = []
    normalized_events = []
    for event in events:
        decisions.append(decision_for_event(event, candidate_types, schema_by_type))
        normalized_events.append(normalize_event(event))

    return json.dumps({"decisions": decisions, "events": normalized_events}, ensure_ascii=False)
        ...
        samples.append(
            {
                ...
                "output": render_output(row["event_mentions"], candidate_types, schema_by_type),
                "meta": {
                    "doc_id": row["doc_id"],
                    "wnd_id": row["wnd_id"],
                    ...
                    "candidate_order_mode": candidate_order_mode,
                    "prompt_style": "contrastive_minimal_cot_v6",
                    **dataset_meta,
                },
            }
        )
