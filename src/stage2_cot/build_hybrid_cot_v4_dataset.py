...
def render_instruction():
    return (
        "Do event extraction with a hybrid structured CoT. "
        ...
        "Each decision may contain only one short `reason` sentence. "
        "Then provide the final extracted events in top-level `events`. "
        "Do not add free-form explanation outside the JSON schema. "
        'If no valid event is supported, return {"clues": [], "decisions": [], "events": []}.'
    )
...
def render_input(row, candidate_types, schema_by_type):
    ...
    return (
        ...
        "- `decisions` maps each clue to one event type and may contain one short `reason` sentence.\n"
...
def short_reason_for_event(event):
    if event["arguments"]:
        return "trigger and roles support this type"
    return "trigger supports this type"
...
def decision_for_event(event, clue_id):
    return {
        ...
        "reason": short_reason_for_event(event),
        "status": "supported",
    }
        ...
        samples.append(
            {
                ...
                "meta": {
                    ...
                    "prompt_style": "hybrid_cot_v4_noexample",
                    **dataset_meta,
                },
            }
        )
...
def main():
    ...
    print(f"wrote {output_path.with_suffix('.meta.json')}")
