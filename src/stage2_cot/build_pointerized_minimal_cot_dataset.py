...
def render_instruction():
    return (
        "Do event extraction with a pointerized minimal structured planning trace. "
        "Use only the provided candidate ids, event types, and schema cards. "
        "Return strict JSON only. "
        "First provide `plans`, where each plan contains one `candidate_id` and its trigger span. "
        "The `candidate_id` must match the numbered candidate list. "
        "Use `candidate_id` 0 only when an extracted event type is not present in the candidate list. "
        "Then provide the final extracted events in top-level `events`. "
        "Do not add free-form explanation outside the JSON schema. "
        'If no valid event is supported, return {"plans": [], "events": []}.'
    )


def render_candidate_list(candidate_types):
    lines = [f"[{idx}] {event_type}" for idx, event_type in enumerate(candidate_types, start=1)]
    return "\n".join(lines)
...
def render_input(row, candidate_types, schema_by_type):
    ...
    candidate_list = render_candidate_list(candidate_types)
    cards = render_schema_cards(candidate_types, schema_by_type)
    return (
        f"Text:\n{row['text']}\n\n"
        f"Tokens:\n{tokens_text}\n\n"
        f"Candidate ids and event types:\n{candidate_list}\n\n"
        f"Reserved pointer id:\n[{MISSING_CANDIDATE_ID}] NOT_IN_CANDIDATE\n\n"
        f"Schema cards:\n{cards}\n\n"
        "Output requirements:\n"
        "- Return exactly one JSON object.\n"
        "- Top-level keys must be `plans` and `events`.\n"
        "- `plans` is a list of event plans.\n"
        "- Each plan contains `candidate_id` and `trigger`.\n"
        "- `candidate_id` must match the numbered candidate list.\n"
        "- Use `candidate_id` 0 only for an extracted type not present in the candidate list.\n"
...
def plan_for_event(event, candidate_index_by_type):
    return {
        "candidate_id": candidate_index_by_type.get(event["event_type"], MISSING_CANDIDATE_ID),
        "trigger": {
...
def render_output(events, candidate_types):
    if not events:
        return json.dumps({"plans": [], "events": []}, ensure_ascii=False)

    candidate_index_by_type = {event_type: idx for idx, event_type in enumerate(candidate_types, start=1)}
    plans = [plan_for_event(event, candidate_index_by_type) for event in events]
...
def build_samples(
    ...
    for row in rows:
        ...
        candidate_index_map = {str(idx): event_type for idx, event_type in enumerate(candidate_types, start=1)}
        fallback_event_types = sorted(
            {event["event_type"] for event in row["event_mentions"] if event["event_type"] not in candidate_types}
        )
        ...
        samples.append(
            {
                ...
                "output": render_output(row["event_mentions"], candidate_types),
                "meta": {
                    "doc_id": row["doc_id"],
                    "wnd_id": row["wnd_id"],
                    "candidate_types": candidate_types,
                    "candidate_id_map": candidate_index_map,
                    "gold_event_types": sorted({ev["event_type"] for ev in row["event_mentions"]}),
                    "pointer_missing_candidate_id": MISSING_CANDIDATE_ID,
                    "pointer_fallback_event_types": fallback_event_types,
                    "num_pointer_fallback_events": len(fallback_event_types),
                    "prompt_style": "pointerized_minimal_cot_v2_noexample",
                    **dataset_meta,
                },
            }
        )
