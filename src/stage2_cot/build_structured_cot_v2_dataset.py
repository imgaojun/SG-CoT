...
def render_instruction():
    return (
        "Do event extraction under the candidate-type constraint. "
        "Return strict JSON only. "
        "Use a short structured reasoning trace and keep the final extracted events in top-level `events`. "
        "Do not add free-form explanation outside the JSON schema. "
        "If no valid event is supported, return {\"reasoning\": [], \"events\": []}."
    )
...
def render_input(row, candidate_types, schema_by_type):
    ...
    return (
        ...
        "- `reasoning` is a short structured list over trigger, event type, focus roles, and status.\n"
...
def reasoning_for_event(event):
    filled_roles = sorted({arg["role"] for arg in event["arguments"]})
    return {
        "trigger": {
...
def render_output(events):
    ...
    for event in events:
        ...
        reasoning.append(reasoning_for_event(event))
        ...
        samples.append(
            {
                ...
                "meta": {
                    ...
                    "prompt_style": "structured_cot_v2_noexample",
                    **dataset_meta,
                },
            }
        )
